...
def run_cv_splits(wandb_args, args, c, wandb_run):
    ...
    for cv_index in range(starting_cv_index, total_n_cv_splits):
        ...
        if "num_mse_loss_unstd" in trainer.test_loss["label"]:
            result = {"data": c.data_set, "missingrate": c.missingrate, "type": c.missingtype,
                      "time": time.time() - start_time,
                      "seed": c.np_seed,
                      "mse": trainer.test_loss["label"]['num_mse_loss_unstd'].item(),
                      "R2": trainer.test_loss["label"]['R2'].item(),
                      }
        else:
            result = {"data": c.data_set, "missingrate": c.missingrate, "type": c.missingtype,
                      "time": time.time() - start_time,
                      "seed": c.np_seed,
                      "accuracy": trainer.test_loss["label"]['cat_accuracy'].item(),
                      "auroc": trainer.test_loss["label"]['auroc'].item(),
                      }
        out = Path("/data/lsw/result/npt_allresult" + str(start_time) +".json")
