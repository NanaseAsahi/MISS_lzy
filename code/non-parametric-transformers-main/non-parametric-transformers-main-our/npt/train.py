...
class Trainer:
    def __init__(
        ...
        self.pretrain_optim = optim.AdamW(model.parameters(), lr=0.0001)
        # Data Loading
    ...
    def per_epoch_pretrain(self, epoch):
        train_loss = self.run_epoch_pretrain(dataset_mode='train', epoch=epoch,
                                             eval_model=False)
    ...
    def train_and_eval(self):
        ...
        # for epoch in range(1, self.pretrain_epoch):
        #     if self.per_epoch_pretrain(epoch=epoch):
        #         break
        ...
        if self.is_distributed and self.c.mp_no_sync != -1:
            ...
            for epoch in range(1,  self.max_epochs + 1):
                if self.per_epoch_train_eval(epoch=epoch):
                    break
    def run_epoch_pretrain(self, dataset_mode, epoch, eval_model=False):
        ...
        for batch_index, batch_dict_ in enumerate(batch_iter):
            ...
            if (eval_model and self.c.debug_eval_row_interactions
                ...
                # Normal execution
                loss = self.run_batch_pretrain(
                    batch_dict_, dataset_mode, eval_model,
                    epoch, print_n, batch_index)
            running_loss += loss
        print(
            f'Epoch: {epoch}, Running Loss: {running_loss}')


    def pretrain_model(self, X_ragged, row_ips, col_ips, nan_mask):
        in_dims = [X_ragged[0].shape[0], len(X_ragged), -1]
        X = [embed(X_ragged[i]) for i, embed in enumerate(self.model.in_embedding)]
        X = torch.stack(X, 1)

        # Compute feature type (cat vs numerical) embeddings, and add them
        if self.model.feature_type_embedding is not None:
            feature_type_embeddings = self.model.feature_type_embedding(
        ...
        if self.model.feature_index_embedding is not None:
            feature_index_embeddings = self.model.feature_index_embedding(
                self.model.feature_indices)
        ...
        if self.model.embedding_layer_norm is not None:
            X = self.model.embedding_layer_norm(X)

        if self.model.embedding_dropout is not None:
            X = self.model.embedding_dropout(X)

        # apply NPT
        X, _, _, _ = self.model.enc((X, row_ips, col_ips, nan_mask))
    ...
    def run_batch_pretrain(self, batch_dict, dataset_mode, eval_model,
                  epoch, print_n, batch_index):
        self.optimizer.zero_grad()
        col_ips = batch_dict['col_ips']
        row_ips = batch_dict['row_ips']
        nan_mask = batch_dict['missmatrix']
        loss = 0
        ...
        if not self.c.data_set_on_cuda:
            ...
            if label_mask_matrix is not None:
                label_mask_matrix = label_mask_matrix.to(
                    device=device, non_blocking=True)
        aug_features_1 = self.pretrain_model(masked_tensors, row_ips, col_ips, nan_mask)
        aug_features_2 = self.pretrain_model(masked_tensors, row_ips, col_ips, nan_mask)
        ...
        outs = self.pretrain_model(masked_tensors, row_ips, col_ips, nan_mask)
        con_outs = outs[:, self.dataset.metadata['num_features']]
        cat_outs = outs[:, self.dataset.metadata['cat_features']]
        cat_outs = self.model.mlp1(cat_outs)
        con_outs = self.model.mlp2(con_outs)
        if len(con_outs) > 0:
            con_outs = torch.cat(con_outs, dim=1)
            con_features_list = []
            for index in self.dataset.metadata['num_features']:
                con_features_list.append(ground_truth_tensors[index])
            batch_X_num = np.concatenate([item[:, [0]].cpu() for item in con_features_list], axis=1)
            # batch_X_num = batch_X_num.cuda()
            batch_X_num = torch.tensor(batch_X_num)
            batch_X_num = batch_X_num.cuda(device=self.c.exp_device)
            num_mask = nan_mask[:, self.dataset.metadata['num_features']]
            con_outs[num_mask == 0] = 0
            batch_X_num[num_mask == 0] = 0
            # l2 = criterion2(con_outs[num_mask == 1], batch_X_num[num_mask == 1])
            l2 = criterion2(con_outs, batch_X_num)
        else:
            l2 = 0
        l1 = 0
        ...
        # 删除最后一列
        cat_features_list = []
        for index in self.dataset.metadata['cat_features']:
            cat_features_list.append(ground_truth_tensors[index])
        new_data_list = [item[:, :-1].cpu() for item in cat_features_list]

        # 还原One-Hot编码
        restored_labels = [np.argmax(item, axis=1) for item in new_data_list]
        batch_X_cat = torch.stack(restored_labels, dim=1)
        batch_X_cat = torch.tensor(batch_X_cat)
        batch_X_cat = batch_X_cat.cuda(device=self.c.exp_device)
        ...
        for j in range(n_cat - 1):
            ...
            cat_mask = nan_mask[:, self.dataset.metadata['cat_features']]
    ...
    def forward_and_loss(
        ...
        if eval_model:
            with torch.no_grad():
                output = self.model(masked_tensors, col_ips=batch_dict['col_ips'], row_ips=batch_dict['row_ips'],
                                    nan_mask=batch_dict['missmatrix'], **extra_args)
        else:
            output = self.model(masked_tensors, col_ips=batch_dict['col_ips'], row_ips=batch_dict['row_ips'], nan_mask=batch_dict['missmatrix'], **extra_args)
