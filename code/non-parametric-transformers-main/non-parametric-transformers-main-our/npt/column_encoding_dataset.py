...
from npt.datasets.News import NewsClassificationDataset
...
from npt.datasets.gas import gasRegressionDataset
from npt.datasets.HI import HIClassificationDataset

from npt.datasets.Gesture import GestureClassificationDataset
from npt.datasets.Letter import LetterClassificationDataset
...
from npt.datasets.temperature import temperatureRegressionDataset
from pathlib import Path
from npt.IPS import sampling, compute_ips
DATASET_NAME_TO_DATASET_MAP = {
    ...
    'gas': gasRegressionDataset,
    'HI': HIClassificationDataset,
    'News': NewsClassificationDataset,
    'Gesture': GestureClassificationDataset,
    'Letter': LetterClassificationDataset,
    'mnist': MNISTDataset,
    'yacht': YachtDataset,
    ...
    'kick': KickDataset,
    'temperature': temperatureRegressionDataset
}
...
METADATA_FIELDS = [
    ...
    'fixed_test_set_index', 'auroc_setting', 'R2']
    ...
    def generate_classification_regression_dataset(self, data_dict):
        ...
        for split_idx, (train_val_test_indices) in enumerate(
            ...
            if split_idx >= c.exp_n_runs:
                ...
                # number of CV folds.
                break
            X = pd.DataFrame(data_dict['data_table'])
            ...
            train_val_test_indices[0] = train_index
            train_val_test_indices[1] = valid_index
            train_val_test_indices[2] = test_index
            col_softmax = nn.Softmax(dim=1)
            row_softmax = nn.Softmax(dim=0)
            datafull = pd.read_csv(
                Path("/data/lsw/data/data/" + c.data_set + "/" + "mcar_" + c.data_set + "_" + str(
                    0.0) + ".csv"))

            datamiss, indicator = sampling(datafull, data_table, self.ips_num, method='feature')
            ips = compute_ips(datamiss[:, :-1], indicator[:, :-1], num=self.ips_num, method='xgb')
            ...
            data_table = torch.tensor(data_table)
            ...
            row_ips = np.concatenate([
                row_ips[train_val_test_indices[0]],
                row_ips[train_val_test_indices[1]],
                row_ips[train_val_test_indices[2]]], axis=0)
            ips = np.concatenate([
                ips[train_val_test_indices[0]],
                ips[train_val_test_indices[1]],
                ips[train_val_test_indices[2]]], axis=0)
            ...
            new_missing_matrix = missing_matrix[
                np.concatenate(
                    [indices for indices in train_val_test_indices])]

            bert_mask_matrix = ~(
                    train_mask_matrix | val_mask_matrix |
                    test_mask_matrix )
            asdas = train_mask_matrix ^ val_mask_matrix ^ test_mask_matrix ^ bert_mask_matrix
            # There should be no overlap between the matrices.
            # Also no entries should be missed. This assert checks for that.
            assert np.array_equal(
                train_mask_matrix ^ val_mask_matrix ^ test_mask_matrix ^ bert_mask_matrix, np.ones((N, D)))
