...
from npt.model.npt_modules import MHSA, MHSA_first
...
class NPTModel(nn.Module):
    ...
    def __init__(self, c, metadata, device=None):
        ...
        for i in range(len(cat_features) - 1):
            categories.append(input_feature_dims[cat_features[i]] - 1)
        self.num_continuous = len(num_features)
        self.num_categories = len(cat_features) - 1
        ...
        self.pt_mlp = simple_MLP(
            [self.dim_hidden * (self.num_continuous + self.num_categories + 1),
             6 * self.dim_hidden * (self.num_continuous + self.num_categories + 1) // 5,
             self.dim_hidden * (self.num_continuous + self.num_categories + 1) // 2])
        self.mlp1 = sep_MLP(self.dim_hidden, self.num_categories, categories)
        self.mlp2 = sep_MLP(self.dim_hidden, self.num_continuous, np.ones(self.num_continuous).astype(int))
    ...
    def forward(self, X_ragged, X_labels=None, eval_model=None, row_ips=None, col_ips=None, nan_mask=None):
        ...
        # apply NPT
        X, _, _, _ = self.enc((X, row_ips, col_ips, nan_mask))

        if X[0].shape[1] == in_dims[0]:
...
class ReshapeToFlat(nn.Module):
    ...
    @staticmethod
    def forward(X):
        return X[0].reshape(1, X[0].size(0), -1), X[1], X[2], X[3]
...
class ReshapeToNested(nn.Module):
    ...
    def forward(self, X):
        return X[0].reshape(X[0].size(1), self.D, -1), X[1], X[2], X[3]
...
class Print(nn.Module):
    ...
    def forward(self, x, row_ips=None, col_ips=None, nan_mask=None):
        print('Debug', x.shape)
        return x
