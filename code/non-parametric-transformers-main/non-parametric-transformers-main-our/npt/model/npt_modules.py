...
class MAB(nn.Module):
    ...
    def forward(self, X, Y, row_ips, col_ips, nan_mask):
        ...
        #     float("-inf"),
        # )
        A = rearrange(A, '(b h) i j -> b h i j', h=self.heads)
        # if (X.shape[0] == 1):
        #     ips = torch.tensor(row_ips).cuda()
        #     ips = ips.unsqueeze(0).unsqueeze(0).unsqueeze(0)
        #     A = A * ips
        # else:
        #     nan_mask = ~nan_mask
        #     nan_mask = nan_mask.cuda()
        #     col_ips = torch.tensor(col_ips)
        #     ones = torch.ones((col_ips.shape[0], 1))
        #     col_ips = torch.cat([col_ips, ones], axis=1)
        #     ips = col_ips.cuda()
        #     ips = ips.unsqueeze(1).unsqueeze(2)
        #     A = A * ips
        #     A = A.masked_fill(
...
class MHSA(nn.Module):
    ...
    def forward(self, X):
        return self.mab(X[0], X[0], X[1], X[2], X[3]),  X[1], X[2], X[3]
class MAB_first(nn.Module):
    """Multi-head Attention Block.
    ...
    def forward(self, X, Y, row_ips, col_ips, nan_mask):
        ...
        A = rearrange(A, '(b h) i j -> b h i j', h=self.heads)
        if (X.shape[0] == 1):
            ips = torch.tensor(row_ips).cuda(device=1)
            ips = ips.unsqueeze(0).unsqueeze(0).unsqueeze(0)
            A = A * ips
        else:
            nan_mask = ~nan_mask
            nan_mask = nan_mask.cuda(device=1)
            col_ips = torch.tensor(col_ips)
            ones = torch.ones((col_ips.shape[0], 1))
            col_ips = torch.cat([col_ips, ones], axis=1)
            ips = col_ips.cuda(device=1)
            ips = ips.unsqueeze(1).unsqueeze(2)
            A = A * ips
            A = A.masked_fill(
                nan_mask.unsqueeze(1).unsqueeze(2),
...
class MHSA_first(nn.Module):
    ...
    def __init__(self, dim_in, dim_emb, dim_out, c):
        super(MHSA_first, self).__init__()
        self.mab = MAB_first(dim_in, dim_in, dim_emb, dim_out, c)
