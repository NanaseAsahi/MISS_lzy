...
class Loss:
    ...
    * Loss.finalize_epoch_losses() to obtain the loss_dict for the current
    ...
    def setup_R2(self, metadata):
        if metadata.get('R2', False):
            num_target_cols = metadata['num_target_cols']
            print('Using R2 in loss module.')
            self.use_R2 = True
            self.R2_col = num_target_cols[0]
            self.reset_R2()
        else:
            print('Disabled R2 in loss module.')
            self.use_R2 = False
    ...
    def finalize_epoch_losses(self, eval_model):
        ...
        if eval_model and self.use_auroc:
            ...
            self.reset_auroc()
        if eval_model and self.use_R2:
            R2 = self.compute_R2()
            std_dict['label']['R2'] = R2
            self.reset_R2()
    ...
    def compute_column_loss(
        ...
        if is_cat:
            ...
            if eval_model and self.use_R2 and (col == self.R2_col):
                self._batch_predictions.append(output[col_mask].detach())
                self._batch_true_vals.append(data[col_mask].detach())
    ...
    def compute_R2(self):
        ...
        if torch.sum(true) == 0:
            ...
            R2 = r2_score(true.cpu(), preds.cpu())
        return R2
    def compute_auroc(self):
        ...
        if torch.sum(true) == 0:
            ...
            try:
                auroc = lightning_auroc(preds[:, 1], true, "binary")
            except:
                auroc = roc_auc_score(true.cpu(), preds.cpu(), multi_class='ovo', average="macro")
