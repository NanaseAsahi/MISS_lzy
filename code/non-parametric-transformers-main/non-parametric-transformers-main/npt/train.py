...
class Trainer:
    ...
    def run_batch_pretrain(self, batch_dict, dataset_mode, eval_model,
        ...
        if not self.c.data_set_on_cuda:
            ...
            if label_mask_matrix is not None:
                label_mask_matrix = label_mask_matrix.to(
                    device=device, non_blocking=True)
        aug_features_1 = self.pretrain_model(masked_tensors)
        aug_features_2 = self.pretrain_model(masked_tensors)
        ...
        outs = self.pretrain_model(masked_tensors)
        ...
        if len(con_outs) > 0:
            ...
            batch_X_num = batch_X_num.cuda(device=device)
        ...
        if len(cat_features_list) != 0:
            ...
            batch_X_cat = batch_X_cat.cuda(device=device)
            n_cat = batch_X_cat.shape[1]
        # 创建(256, 1)的数组
            for j in range(n_cat - 1):
                l1 += criterion1(cat_outs[j], batch_X_cat[:, j])
        loss += 1 * l1 + 1 * l2
        loss.backward()
        self.optimizer.step()
        return loss

    def pretrain_model(self, X_ragged):
