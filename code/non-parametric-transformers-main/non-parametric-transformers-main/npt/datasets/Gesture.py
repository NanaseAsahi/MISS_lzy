...
class GestureClassificationDataset(BaseDataset):
    def __init__(self, c):
        super(GestureClassificationDataset, self).__init__(
            fixed_test_set_index=None)
        self.c = c

    def load(self):
        (self.data_table, self.N, self.D, self.cat_features, self.num_features,
            self.missing_matrix) = load_and_preprocess_Gesture_dataset(
            self.c)
...
def load_and_preprocess_Gesture_dataset(c):
    ...
    data_table = data_table.fillna(np.nanmean(data_table))
