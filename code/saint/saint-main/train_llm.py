...
from models.pretrainmodel_gpt import SAINT
from transformers.models.gpt2.modeling_gpt2 import GPT2Model
from data_openml import data_prep_openml,task_dset_ids,DataSetCatCon,Dataset_Custom
import argparse
...
bi_datasets = ["HI"]
missingtypes = ["mcar_"]
ips_nums = [20, 40, 60, 80, 100, 120, 140, 160, 180, 200]
...
def run(runs, dataset_name, missingrate, task, missingtype, ips_num):
    ...
    #     configs=opt
    # )
    model = GPT4TS(opt, device,categories=tuple(cat_dims),
        num_continuous=len(con_idxs),
        dim=opt.embedding_size,
        dim_out=1,
        depth=opt.transformer_depth,
    ...
    for epoch in range(1000):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            if opt.attentiontype == 'col':
                reps = model(x_categ_enc, x_cont_enc, con_mask, cat_mask, cat_ips, con_ips)
            else:
                reps = model(torch.cat((x_categ_enc, x_cont_enc), dim=1))
            # if opt.attentiontype == 'col':
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pretrain_gpt', type=int, default=1)
    ...
    parser.add_argument('--attentiontype', default='justmlp', type=str,
