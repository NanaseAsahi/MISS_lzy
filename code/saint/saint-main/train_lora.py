...
from models.pretrainmodel_lora import SAINT, Lora

from data_openml import data_prep_openml,task_dset_ids,DataSetCatCon
import argparse
from torch.utils.data import DataLoader
import torch.optim as optim
from utils_lora import count_parameters, classification_scores, mean_sq_error
...
def run(runs, dataset_name, missingrate, task, missingtype, ips_num, rank):
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            reps_lora = model.lora(x_categ_enc, x_cont_enc)
            # if opt.attentiontype == 'col':
            #     cat_outs, con_outs = model(x_categ_enc, x_cont_enc, con_mask, cat_mask, cat_ips, con_ips)
            # else:
            #     cat_outs, con_outs = model(x_caateg_enc, x_cont_enc, con_mask, cat_mask, cat_ips, con_ips, row_ips)
            ...
            con_lora = torch.chunk(reps_lora[:, model.num_categories:].t().unsqueeze(2), reps_lora[:, model.num_categories:].size(1), dim=0)
            con_lora = [t.squeeze(0) for t in con_lora]
            cat_lora = torch.chunk(reps_lora[:, :model.num_categories].t().unsqueeze(2),
                                   reps_lora[:, :model.num_categories].size(1), dim=0)
            cat_lora = [t.squeeze(0) for t in cat_lora]
            cat_outs = [t1 + t2 for t1, t2 in zip(cat_outs, cat_lora)]
            con_outs = [t1 + t2 for t1, t2 in zip(con_lora, con_outs)]
            # cat_outs = model.mlp1(reps[:, :model.num_categories, :]) + reps_lora[:, :model.num_categories]
            # con_outs = model.mlp2(reps[:, model.num_categories:, :]) + reps_lora[:, model.num_categories:]
            y_outs = model.mlpfory(y_reps) + model.mlpfory_lora(reps_lora[:,0])
            if task == 'regression':
    ...
    print("finetune_cost_time", time.time() - starttime)
        # if opt.active_log:
    ...
    # plt.show()
    return best_test_auroc_list, best_test_accuracy_list, pretrain_end_time - pretrain_start_time, time.time() - starttime
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pt_tasks', default=['mask'], type=str, nargs='*',
            ...
            for missingtype in missingtypes:
                for ips_num in ips_nums:
                    for rank in ranks:
                        ...
                        for runs in range(3):
                            starttime = time.time()
                            best_test_auroc_list, best_test_accuracy_list, pretrain_time, finetune_time = run(runs, data, missingrate, "binary", missingtype, ips_num, rank)
                            print(best_test_accuracy_list)
                            pretrain_time_list.append(pretrain_time)
                            fine_tune_time_list.append(finetune_time)
                            print( "time", time.time() - starttime)
                        result = {"data": data, "missingrate": missingrate, "task": "binary", "type": missingtype, "ips_num": ips_num,
                                  "pretrain_time": pretrain_time_list, "finetune_time": fine_tune_time_list,
                                  "seed": seeds,
                                  "best_test_auroc_list": best_test_auroc_list,
                                  "best_test_accuracy_list": best_test_accuracy_list,"rank":rank}
            ...
            for missingtype in missingtypes:
                for ips_num in ips_nums:
                    for rank in ranks:
                        ...
                        for runs in range(3):
                            starttime = time.time()
                            best_test_auroc_list, best_test_accuracy_list, pretrain_time, finetune_time = run(runs, data, missingrate, "multiclass", missingtype, ips_num, rank)
                            pretrain_time_list.append(pretrain_time)
                            fine_tune_time_list.append(finetune_time)
            ...
            for missingtype in missingtypes:
                for ips_num in ips_nums:
                    for rank in ranks:
                        ...
                        for runs in range(3):
                            starttime = time.time()
                            best_test_auroc_list, best_test_accuracy_list, pretrain_time, finetune_time = run(runs, data,
                                                                                                          missingrate,
                                                                                                          "regression",
                                                                                                          missingtype, ips_num, rank)
                            pretrain_time_list.append(pretrain_time)
                            fine_tune_time_list.append(finetune_time)
                        result = {"data": data, "missingrate": missingrate, "task": "regression", "type": missingtype, "ips_num": ips_num,
                                  ...
                                  "best_test_R2_list": best_test_accuracy_list, "rank": rank}
