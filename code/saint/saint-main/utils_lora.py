...
#计算accuracy和auc
def classification_scores(model, dloader, device, task, vision_dset, attentiontype):
    model.train()
    m = nn.Softmax(dim=1)
    y_test = torch.empty(0).to(device)
    y_pred = torch.empty(0).to(device)
    ...
    with torch.no_grad():
        for i, data in enumerate(dloader, 0):
            x_categ, x_cont, y_gts, cat_mask, con_mask, cat_ips, con_ips, rowips = data[0].to(device), data[1].to(device),data[2].to(device),data[3].to(device),data[4].to(device),data[5].to(device),data[6].to(device),data[7].to(device)
            _ , x_categ_enc, x_cont_enc = embed_data_mask(x_categ, x_cont, cat_mask, con_mask,model,vision_dset)           
            ...
            if task == 'binary':
                ...
                prob = torch.cat([prob, m(y_outs).float()], dim=0)
