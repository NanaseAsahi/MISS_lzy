...
def data_split_label(X, y, nan_mask, indices):
    combined_array = np.hstack((X.values[indices], y[indices].reshape(-1, 1)))
    nan_mask = nan_mask.values[indices]
    # 根据最后一列是否为空来过滤样本
    non_empty_rows = combined_array[~np.isnan(combined_array[:, -1])]
    x_d = {
        'data': non_empty_rows[:, :-1],
        'mask': nan_mask[~np.isnan(combined_array[:, -1])]
    }
    ...
    y_d = {
        'data': non_empty_rows[:, -1].reshape(-1, 1)
    }
    return x_d, y_d

def data_prep_openml(dataset, missingrate, seed, task, datasplit=[.65, .15, .2], missingtype="mcar_", imp=None):
    ds_id = 42178
    np.random.seed(seed)
    ...
    # out = Path("/data/lsw/representations/data/" + dataset + "/" + missingtype + dataset + "_" + str(missingrate) + ".csv")
    if imp != "mean":
        out = Path("/data/lsw/data/data/" + dataset + "/" + missingtype + dataset + "_" + str(
            missingrate) + imp + ".csv")
    else:
        out = Path(
            "/data/lsw/data/data/" + dataset + "/" + missingtype + dataset + "_" + str(missingrate) + ".csv")
    # out = Path("/data/lsw/representations/data/OpenDEL0001_fringerprint_256.csv")
...
def data_prep_openml_label(dataset, missingrate, seed, task, datasplit=[.65, .15, .2], missingtype="mcar_", imp=None):
    ds_id = 42178
    np.random.seed(seed)
    # out = Path("/home/pyc/workspace/saint-main/" + dataset + "/" + "mcar_" + dataset + "_" + str(missingrate) + ".csv")
    # out = Path("/data/lsw/missingdata/data/" + dataset + "/" + "mcar_" + dataset + "_" + str(missingrate) + ".csv")
    ...
    # if task != 'regression':
    #     y = y.astype(int)
    X_train, y_train = data_split_label(X, y, nan_mask, train_indices)
    X_valid, y_valid = data_split_label(X, y, nan_mask, valid_indices)
    X_test, y_test = data_split_label(X, y, nan_mask, test_indices)
