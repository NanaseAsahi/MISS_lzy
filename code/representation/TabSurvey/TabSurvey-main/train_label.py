...
from utils.load_data_label import load_data
...
def cross_validation(model, X, y, args, save_model=False):
    ...
    # 如果你想删除带有空值的样本，可以使用 dropna
    # non_empty_rows = combined_array[~np.isnan(combined_array).any(axis=1)]
    X_train, y_train = non_empty_rows[:, :-1], non_empty_rows[:, -1]
...
def main_once(args):
    ...
    types = ["mcar_label_"]
    imputations = ["noimp"]  # noimp表示不补全，xgboost可以不补全直接运行
                ...
                for model_str_name in models:
                    for rate in missing_rate:
                        ...
                        for seed in seeds:
                            ...
                            out = Path("/data/lsw/result/representation_allresult_label" + t_time + ".json")
