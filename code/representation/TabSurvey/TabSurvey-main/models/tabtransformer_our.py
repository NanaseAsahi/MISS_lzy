...
class TabTransformer(BaseModelTorch):
    ...
    def __init__(self, params, args):
        ...
        self.to_device()
    def tab_ori_pretrain(self, model, X, y, nan_mask, ips):
        train_dataset = TensorDataset(X, y, nan_mask, ips)
        ...
        for epoch in range(20):
            running_loss = 0.0
            for i, (batch_X, batch_y, batch_nan_mask, batch_ips) in enumerate(train_loader):
                optimizer.zero_grad()
                loss = 0

                if self.args.cat_idx:
                    x_categ = batch_X[:, self.args.cat_idx].int().to(self.device)
                    x_categ_mask = batch_nan_mask[:, self.args.cat_idx].int().to(self.device)
                    x_categ_ips = batch_ips[:, self.args.cat_idx].int().to(self.device)
                else:
                    x_categ = None
                    x_categ_mask = None
                    x_categ_ips = None
                ...
                if x_categ is not None:
                    aug_features_1 = model.transformer(x_categ, x_categ_mask, x_categ_ips)
                    aug_features_2 = model.transformer(x_categ, x_categ_mask, x_categ_ips)
                    ...
                    cat_outs = model.transformer(x_categ, x_categ_mask, x_categ_ips)
            ...
            if running_loss < min_pretrain_loss:
                min_pretrain_loss = running_loss
                min_pretrain_loss_idx = epoch

                # Save the currently best model
                self.save_model(filename_extension="best", directory="tmp")

            if min_pretrain_loss_idx + self.args.early_stopping_rounds < epoch:
                print("loss has not improved for %d steps!" % self.args.early_stopping_rounds)
                print("Early stopping applies.")
                break
        return model
    def fit(self, X, y, X_val=None, y_val=None, ips=None, ips_val=None):
        learning_rate = 10 ** self.params["learning_rate"]
        weight_decay = 10 ** self.params["weight_decay"]
        optimizer = optim.AdamW(self.model.parameters(), lr=learning_rate, weight_decay=weight_decay)
        nan_mask = ~np.isnan(X)
        nan_mask = nan_mask.astype(int)
        X = pd.DataFrame(X)
        X = X.fillna(np.mean(X))
        X = np.array(X)
        val_nan_mask = ~np.isnan(X_val)
        val_nan_mask = val_nan_mask.astype(int)
        X_val = pd.DataFrame(X_val)
        X_val = X_val.fillna(np.mean(X_val))
        ...
        ips = torch.tensor(ips).float()
        ips_val = torch.tensor(ips_val).float()
        nan_mask = torch.tensor(nan_mask)
        val_nan_mask = torch.tensor(val_nan_mask)
        y = torch.tensor(y)
        ...
        val_dataset = TensorDataset(X_val, y_val, val_nan_mask, ips_val)
        val_loader = DataLoader(dataset=val_dataset, batch_size=self.args.val_batch_size, shuffle=True)

        min_val_loss = float("inf")
        min_val_loss_idx = 0
        model = self.tab_ori_pretrain(self.model, X, y, nan_mask, ips)
        self.model = model
        loss_history = []
        val_loss_history = []
        ...
        for epoch in range(self.args.epochs):
            for i, (batch_X, batch_y, batch_nan_mask, batch_ips) in enumerate(train_loader):
                ...
                out = self.model(x_categ, x_cont, x_categ_mask, x_categ_ips)
            ...
            val_loss = 0.0
            val_dim = 0
            for val_i, (batch_val_X, batch_val_y, batch_val_nan_mask, batch_val_ips) in enumerate(val_loader):

                if self.args.cat_idx:
                    x_categ = batch_val_X[:, self.args.cat_idx].int().to(self.device)
                    x_categ_mask = batch_val_nan_mask[:, self.args.cat_idx].int().to(self.device)
                    x_categ_ips = batch_val_ips[:, self.args.cat_idx].int().to(self.device)
                else:
                    x_categ = None
                    x_categ_mask = None
                    x_categ_ips = None
    ...
    def predict_helper(self, X, ips):
        ...
        test_dataset = TensorDataset(X, nan_mask, ips)
        ...
        with torch.no_grad():
            for batch_X, batch_nan_mask, batch_ips in test_loader:
                # print(batch_ips.shape)
                # print(batch_X.shape)
                # print(batch_nan_mask.shape)
                # print(self.args.cat_idx)
                x_categ = batch_X[:, self.args.cat_idx].int().to(self.device) if self.args.cat_idx else None
                x_cont = batch_X[:, self.num_idx].to(self.device)
                x_categ_mask = batch_nan_mask[:, self.args.cat_idx].int().to(self.device)
                x_categ_ips = batch_ips[:, self.args.cat_idx].int().to(self.device)
                preds = self.model(x_categ, x_cont, x_categ_mask, x_categ_ips)
...
class Lora(nn.Module):
    def __init__(self, m, n, rank=10):
        super().__init__()
        self.m = m
        self.A = nn.Parameter(torch.randn(m, rank)).to("cuda:0")
        self.B = nn.Parameter(torch.zeros(rank, n)).to("cuda:0")

    def forward(self, x):
        x = x.view(-1, self.m)
        # 创建新的tensor，并指定设备，然后转换为nn.Parameter
...
class TabTransformerModel(nn.Module):
    def __init__(
        ...
        categories_offset = F.pad(torch.tensor(list(categories)), (1, 0),
                                  value=num_special_tokens)  # Prepend num_special_tokens.
        ...
        self.mlpfory_lora = simple_MLP([1, 1000, dim_out])
        self.norm = nn.LayerNorm(num_continuous)
        self.num_continuous = num_continuous
        nfeat = num_continuous + len(categories)
        ...
        try:
            self.pt_mlp = simple_MLP(
                [dim * (self.num_categories), 6 * dim * (self.num_categories) // 5, dim * (self.num_categories) // 2])
    ...
    def forward(self, x_categ, x_cont, x_categ_mask, x_categ_ips):
        ...
        if x_categ is not None:
            ...
            x_categ = x_categ.float()
            x_lora = self.lora(x_categ)
