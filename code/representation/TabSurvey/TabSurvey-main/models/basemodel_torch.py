...
class BaseModelTorch(BaseModel):
    ...
    def predict_ips(self, X, ips=None):
        if self.args.objective == "regression":
            self.predictions = self.predict_helper(X, ips=ips)
        else:
            self.predict_proba_ips(X, ips=ips)
            self.predictions = np.argmax(self.prediction_probabilities, axis=1)

        return self.predictions

    def predict_proba_ips(self, X: np.ndarray, ips=None) -> np.ndarray:
        probas = self.predict_helper(X, ips=ips)
