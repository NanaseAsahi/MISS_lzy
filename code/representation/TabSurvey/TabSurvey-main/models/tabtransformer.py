...
class TabTransformer(BaseModelTorch):
    ...
    def tab_ori_pretrain(self, model, X, y):
        ...
        for epoch in range(20):
            ...
            for i, (batch_X, batch_y) in enumerate(train_loader):
                ...
                if(x_categ is not None):
                    aug_features_1 = model.module.transformer(x_categ)
                    aug_features_2 = model.module.transformer(x_categ)
                    ...
                    aug_features_1 = model.module.pt_mlp(aug_features_1)
                    aug_features_2 = model.module.pt_mlp(aug_features_2)
                    nce_temp = 0.7
                    logits_per_aug1 = aug_features_1 @ aug_features_2.t() / nce_temp
                    logits_per_aug2 = aug_features_2 @ aug_features_1.t() / nce_temp
                    targets = torch.arange(logits_per_aug1.size(0)).to(self.device)
                    ...
                    cat_outs = model.module.transformer(x_categ)
                    cat_outs = model.module.mlp1(cat_outs)
    ...
    def fit(self, X, y, X_val=None, y_val=None):
        ...
        min_val_loss_idx = 0
        model = self.tab_ori_pretrain(self.model, X, y)
        self.model = model
        loss_history = []
        val_loss_history = []
...
class TabTransformerModel(nn.Module):
    def __init__(
        ...
        try:
            self.pt_mlp = simple_MLP([dim*(self.num_categories) ,6*dim*(self.num_categories)//5, dim*(self.num_categories)//2])
