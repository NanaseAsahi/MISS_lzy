...
def notMIWAE(data, S, odata):
    ...
    def miwae_loss(iota_x, mask):
        ...
        all_means_obs_model = rearrange(all_means_obs_model, '(b n) d -> b n d', n=K)

        s = mask.unsqueeze(1)
        iota_x = iota_x.unsqueeze(1)

        l_out_mixed = all_means_obs_model * (1 - s) + (iota_x * s)
        l_out_mixed = l_out_mixed.permute(1, 0, 2)
        W = nn.Parameter(torch.randn(1, 1, D)).cuda()
        W = -F.softplus(W)
        b = nn.Parameter(torch.randn(1, 1, D)).cuda()

        logits = W * (l_out_mixed - b)

        p_s_given_x = torch.distributions.Bernoulli(logits=logits).log_prob(s.unsqueeze(1))
        lpsx = torch.sum(p_s_given_x, dim=-1)
        neg_bound = -torch.mean(torch.logsumexp(logpxobsgivenz + logpz + lpsx - logq,0))
    ...
    def miwae_impute(iota_x,mask,L):
        ...
        all_means_obs_model = rearrange(all_means_obs_model, '(b n) d -> b n d', n=L)
        ...
        logits = W * (l_out_mixed - b)
        logits = rearrange(logits, 'b n d -> (b n) d', b=L)
        p_s_given_x = torch.distributions.Bernoulli(logits=logits).log_prob(tiledmask)
        lpsx = torch.sum(p_s_given_x, 1).reshape([L,batch_size])
        imp_weights = torch.nn.functional.softmax(logpxobsgivenz + logpz + lpsx - logq,0) # these are w_1,....,w_L for all observations in the batch
    ...
    for ep in range(1,n_epochs):
        ...
        batches_s = np.array_split(S[perm,], n/bs)
        losses = []
        for it in range(len(batches_data)):
            ...
            b_s = torch.from_numpy(batches_s[it]).float().cuda()
