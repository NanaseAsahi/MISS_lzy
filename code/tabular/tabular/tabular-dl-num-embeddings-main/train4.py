...
class NonFlatModel(BaseModel):
    def __init__(self, config: Config, dataset, n_bins: Optional[int]):
        ...
        self.pretrain_module = rtdl.pretrain_Transformer(
            d_token=d_embedding,
            **transformer_options,
            d_out=dataset.nn_output_dim,
        )
...
def pretrain_module(model, x: Tensor):
                    ...
                    for imp in imputations:
                            ...
                            for epoch in stream.epochs(C.training.n_epochs):
                                ...
                                for batch_idx in epoch:
                                    ...
                                    x = torch.concat([x_ for x_ in x if x_ is not None], 1)
                                    aug_features_1 = pretrain_module(model, x)
                                    aug_features_2 = pretrain_module(model, x)
                                    aug_features_1 = (
                                    ...
                                    outs = pretrain_module(model, x)
                                    con_outs = outs[:, :D.n_num_features]
                                    outs = pretrain_module(model, x)
                                    con_outs = outs[:, :D.n_num_features]
                                    if imp == "mean" and X_cat is not None:
                            ...
                            out = Path("/data/lsw/result/tabular_ori_allresult" + t_time + ".json")
