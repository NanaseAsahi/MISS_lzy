...
def evaluate(parts):
    ...
    for part in parts:
        while eval_batch_size:
            try:
                predictions[part], ximp[part]= (
                    torch.cat(
                        [
                            apply_model_pred(part, idx)
...
def objective_function(trial):
    ips_num = trial.suggest_discrete_uniform('ips_num', 20, 40, 60, 80, 100, 120, 140, 160, 180, 200)
    l1 = trial.suggest_uniform('l1', 0, 10)
    l2 = trial.suggest_uniform('l2', 0, 10)
    mask_rate = trial.suggest_uniform('mask_rate', 0, 0.3)
            ...
            for missingtype in missingtypes:
                for ips_num in ips_nums:
                    for seed in seeds:
                        ...
                        # 标记专家数据
                        datamiss, indicator = sampling(datafull, datamiss, int(C.data.ips_num), method='feature')
                        ips = compute_ips(datamiss[:, :-1], indicator[:, :-1], num=int(C.data.ips_num), method='xgb')
                        ...
                        D = lib.build_dataset(C.data.path, C.data.T, C.data.T_cache, num_nan_mask_dict, cat_nan_mask_dict, C.data.mask_rate)
                            ...
                            for batch_idx in epoch:
                                ...
                                if D.X_cat is not None:
                                    ...
                                    for j in range(1, n_cat):
                                        ...
                                        log_x[cat_mask[:, j] == 0] = 0
                                        l1 += abs(sum(log_x) / cat_outs[j].shape[0])
                                loss += C.model.l1 * l1 + C.model.l2 * l2
                                loss.backward()
                                optimizer.step()
