...
def split_train_test_val(X, y, X_num, X_cat, seed=seed):
    # csv文件中最后一行均为label/y
    ...
    X_copy['Set'] = np.random.choice(['train', 'test', 'val'], p=[.8, .1, .1], size=X_copy.shape[0])
    train_index = X_copy[X_copy['Set'] == 'train'].index
    test_index = X_copy[X_copy['Set'] == 'test'].index
    val_index = X_copy[X_copy['Set'] == 'val'].index
        ...
        for missingtype in missingtypes:
            for missingrate in missingrates:
                ...
                if csv_path.exists():
                    try:
                        # 将所有操作抽象成对单个数据集的操作
                        info = process_single_dataset(csv_path, output_dir, seed=0)
                        print(f'成功处理！保存到 {output_dir}')
