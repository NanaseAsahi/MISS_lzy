...
class NonFlatModel(BaseModel):
    def __init__(self, config: Config, dataset, n_bins: Optional[int]):
        ...
        self.mlpfory_lora = simple_MLP([1, 1000, dataset.nn_output_dim])
        self.lora = Lora((dataset.n_features + 1) * d_embedding, (dataset.n_features + 1))
        ...
        while eval_batch_size:
            try:
                ...
                for idx in index_loader:
                    # apply_model返回两个张量
                    _predictions, x_imp = apply_model_ft(part, idx)
...
missingrates = [0.5]
datasets = ["temperature", 'Gesture', "HI", "News"]
missingtypes = ["mcar_"]
imputations = ["mean"]
                ...
                for imp in imputations:
                    for ips_num in ips_nums:
                        for seed in seeds:
                            ...
                            if missingtype == "mnar_":
                                config_path = "exp/transformer-q-lr/" + data_name + "/" + "mnar_p_" + str(
                                    0.5) + ".toml"
                            elif missingtype == "mar_":
                                config_path = "exp/transformer-q-lr/" + data_name + "/" + "mar_p_" + str(
                                    0.5) + ".toml"
                            else:
                                ...
                                for batch_idx in epoch:
                                    ...
                                    while chunk_size != 0:
                                        try:
                                            ...
                                            optimizer.zero_grad()
                                            (y_pred, x_feature), y = apply_model_ft('train', batch_idx), Y['train'][
                                                batch_idx]
                                            x = model._encode_input((encode('train', batch_idx)  ),None if X_cat is None else batch_X_cat,)
                                            if x[1] is not None:
                                                x = torch.cat((x[0], x[1]), dim=1)
                                            else:
                                                x = x[0]
                                            x = model.cls_embedding(x)
                                            # lora_reps = model.lora(x)
                                            # y_lora_reps = lora_reps[:, -1]
                                            # x_lora_reps = lora_reps[:, :-1]
